...
void concat_with_pc()
{
    while (ros::ok())
    {
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/laser0", ros::Duration(1));
        if (sharedPtr == NULL)
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/laser1", ros::Duration(1));
        if (sharedPtr == NULL)
            ROS_INFO("No laser messages received");
        else
            cloud_2 = *sharedPtr;
